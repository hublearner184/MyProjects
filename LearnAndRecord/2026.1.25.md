...
强调一下动态数组的知识点：
1.实现了动态扩容和缩容；2.元素之间搬移逻辑：删除或者增加必须处理元素的移动；3.模板与泛型编程template<typename E>使这个类具有了通用性。4.内存管理：C++程序员必须手动管理堆内存，包括构造函数的申请和析构函数的释放，将对象的生存周期与资源绑定，防止内存泄漏。5.区分了两种索引检查分别适用查找增删。
...
这里主要实现了单链表的增加，删除比较简单就不说了。（删除时要注意的是被删除节点记得将其指针释放即设置为nullptr，这是个良好的习惯）
